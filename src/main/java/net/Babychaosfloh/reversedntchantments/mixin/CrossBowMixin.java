...
public class CrossBowMixin {
    @Unique
    private static int reversedMultishotLvl;
    @Unique
    private static int reversedInfinityLvl;
    @Unique
    private static boolean REVERSED_MULTISHOT;
    @Unique
    private static boolean REVERSED_INFINITY;
    ...
    private static boolean loadProjectile(LivingEntity pShooter, ItemStack pCrossbowStack, ItemStack pAmmoStack, boolean pHasAmmo, boolean pIsCreative) {
        REVERSED_MULTISHOT = ReversedEnchantmentsServerConfigs.ENCHANTMENTS.get("reversed_multishot").get();
        REVERSED_INFINITY = ReversedEnchantmentsServerConfigs.ENCHANTMENTS.get("reversed_infinity").get();
        ...
        if (pAmmoStack.isEmpty()) {
            ...
            ItemStack itemstack;
            reversedMultishotLvl = REVERSED_MULTISHOT ? EnchantmentHelper.getItemEnchantmentLevel(ModEnchantments.REVERSED_MULTISHOT.get(), pCrossbowStack) : 0;
            reversedInfinityLvl = REVERSED_INFINITY ? EnchantmentHelper.getItemEnchantmentLevel(ModEnchantments.REVERSED_INFINITY.get(), pCrossbowStack) : 0;

            pShooter.sendSystemMessage(Component.literal("negMult "+ reversedMultishotLvl));
            pShooter.sendSystemMessage(Component.literal("negInf "+ reversedInfinityLvl));

            if (!flag && !pIsCreative && !pHasAmmo) {
                int splitter = 0;
                if (reversedInfinityLvl > 0) {
                    splitter = reversedInfinityLvl;
                } else if (reversedMultishotLvl > 0) {
                    splitter = reversedMultishotLvl * 3;
                } else {
                    splitter = 1;
                }
